...
local function find_executable(dir)
  ...
  local function not_in_cmakefiles(p)
    return not string.find(p, "/CMakeFiles/")
  end

  local all = vim.fn.globpath(dir, "**/*", true, true)
  for _, p in ipairs(all) do
    if not_in_cmakefiles(p) and vim.loop.fs_stat(p) and vim.fn.executable(p) == 1 then
      return p
    end
  end
